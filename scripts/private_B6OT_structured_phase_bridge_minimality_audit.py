...
Question:
    Which minimal ingredients of the structured GMR phase bridge are required
    for the B6O-R intersection-access effect?

Boundary:
    C is kept fixed as side+tfc+phase. FES is not added as a state label.
    FES/GMR is used only inside the bridge operation. This is not an FES-label
    audit and not a new definition of C.
"""
...
BRIDGE_TARGET_STEPS = {
    "true72_forward": 1.0,
    "reversed72_no_forward": -1.0,
    "permuted_fes_order": 1.0,
    "linear_noncyclic": 1.0,
    "half36_fixed": 0.5,
    "quadrature90_fixed": 1.25,
    "skip144_fixed": 2.0,
}

# Deterministic permutation that preserves five bins but breaks the original
# Activation -> Challenge -> Surprise -> SelfGrowth -> CoCreation order.
PERMUTED_FES_INDEX = np.asarray([0, 2, 4, 1, 3], dtype=float)

BRIDGE_MINIMALITY_FACTORS = {
    "true72_forward": {
        "phase_structure": True,
        "cyclic_order": True,
        "fixed_angular_step": True,
        "fes_five_string_order": True,
    ...
    "reversed72_no_forward": {
        ...
        "ablation": "remove_forward_orientation",
    },
    "permuted_fes_order": {
        "phase_structure": True,
        "cyclic_order": True,
        "fixed_angular_step": True,
        "fes_five_string_order": False,
        "forward_orientation": True,
        "ablation": "remove_fes_five_string_order",
    },
    "linear_noncyclic": {
        "phase_structure": True,
        "cyclic_order": False,
        ...
        "fes_five_string_order": True,
        "forward_orientation": True,
        "ablation": "remove_cyclic_wrap",
    },
    "variable_step_structured": {
        "phase_structure": True,
        "cyclic_order": True,
        "fixed_angular_step": False,
        "fes_five_string_order": True,
        "forward_orientation": False,
        "ablation": "remove_fixed_step",
    },
    "half36_fixed": {
        "phase_structure": True,
        "cyclic_order": True,
    ...
    "random_phase": {
        ...
        "fes_five_string_order": False,
        "forward_orientation": False,
        "ablation": "remove_structured_phase",
    },
    "no_bridge": {
        "phase_structure": False,
        "cyclic_order": False,
...
def bridge_resonance(
    step: float,
    variant: str,
    rng: np.random.Generator | None = None,
    raw_step: float | None = None,
    current_fes: float | None = None,
    future_fes: float | None = None,
) -> float:
    ...
    if not np.isfinite(step):
        return np.nan
    if variant == "permuted_fes_order":
        if current_fes is None or future_fes is None or not np.isfinite(current_fes) or not np.isfinite(future_fes):
            return np.nan
        c = PERMUTED_FES_INDEX[int(current_fes) % 5]
        f = PERMUTED_FES_INDEX[int(future_fes) % 5]
        permuted_step = (f - c) % 5.0
        return float(np.cos(2.0 * np.pi * ((permuted_step - 1.0) % 5.0) / 5.0))
    if variant == "linear_noncyclic":
        if raw_step is None or not np.isfinite(raw_step):
            return np.nan
        # Linear Gaussian-like match to +1 without cyclic wrap. This preserves
        # ordered FES distance but removes circular phase closure.
        return float(2.0 * np.exp(-0.5 * ((raw_step - 1.0) / 1.0) ** 2) - 1.0)
    if variant == "variable_step_structured":
        targets = np.asarray([0.5, 1.0, 1.25, 2.0], dtype=float)
        vals = np.cos(2.0 * np.pi * ((step - targets) % 5.0) / 5.0)
    ...
    for _, sub in out.groupby("label", sort=False):
        ...
        for local_i, row_index in enumerate(idxs):
            ...
            for op in OPERATORS:
                ...
                if np.isfinite(current_fes) and np.isfinite(future_fes):
                    raw_step = future_fes - current_fes
                    step = raw_step % 5.0
                else:
                    raw_step = np.nan
                    step = np.nan
                ...
                for variant in BRIDGE_VARIANTS:
                    resonance = bridge_resonance(step, variant, rng, raw_step=raw_step, current_fes=current_fes, future_fes=future_fes)
...
def aggregate(access_summary: pd.DataFrame) -> pd.DataFrame:
    agg = access_summary.groupby("bridge_variant", as_index=False).agg(
    ...
    for _, row in agg.iterrows():
        factors = BRIDGE_MINIMALITY_FACTORS[str(row["bridge_variant"])]
        factor_rows.append({**row.to_dict(), **factors})
    return pd.DataFrame(factor_rows).sort_values(
        ["strong_accesses", "robust_accesses", "mean_true_intersection"],
        ascending=[False, False, False],
    )


def factor_ablation_summary(variant_agg: pd.DataFrame, bridge_comp: pd.DataFrame) -> pd.DataFrame:
    true_mean = float(variant_agg.loc[variant_agg["bridge_variant"].eq("true72_forward"), "mean_true_intersection"].iloc[0])
    comp = bridge_comp.copy()
    comp["bridge_variant"] = comp["comparison"].str.replace("true72_forward_vs_", "", regex=False)
    rows = []
    for _, row in variant_agg.iterrows():
        variant = str(row["bridge_variant"])
        factors = BRIDGE_MINIMALITY_FACTORS[variant]
        if variant == "true72_forward":
            rows.append({
                "bridge_variant": variant,
                "ablation": factors["ablation"],
                "removed_condition": "none",
                "mean_true_intersection": row["mean_true_intersection"],
                "delta_from_true72": 0.0,
                "p_true72_greater": np.nan,
                "robust_accesses": row["robust_accesses"],
                "strong_accesses": row["strong_accesses"],
                **factors,
            })
            continue
        matched = comp[comp["bridge_variant"].eq(variant)]
        p = float(matched["p_greater"].iloc[0]) if not matched.empty else np.nan
        removed = ",".join(k for k in ["phase_structure", "cyclic_order", "fixed_angular_step", "fes_five_string_order", "forward_orientation"] if not bool(factors[k]))
        rows.append({
            "bridge_variant": variant,
            "ablation": factors["ablation"],
            "removed_condition": removed or "step_changed_only",
            "mean_true_intersection": row["mean_true_intersection"],
            "delta_from_true72": true_mean - float(row["mean_true_intersection"]),
            "p_true72_greater": p,
            "robust_accesses": row["robust_accesses"],
            "strong_accesses": row["strong_accesses"],
            **factors,
        })
    return pd.DataFrame(rows).sort_values("delta_from_true72", ascending=False)


def write_report(path: Path, access_summary: pd.DataFrame, bridge_comp: pd.DataFrame, variant_agg: pd.DataFrame, ablation_summary: pd.DataFrame, args: argparse.Namespace) -> None:
    ...
    lines = [
        ...
        "",
        ablation_summary.to_csv(index=False).strip(),
        "",
...
def run(args: argparse.Namespace) -> None:
    ...
    table.to_csv(outdir / "private_B6OT_bridge_variant_state_table.csv", index=False)
    access_frames = []
    policy_frames = []
    for variant in BRIDGE_VARIANTS:
    ...
    ablation_summary = factor_ablation_summary(variant_agg, bridge_comp)

    access.to_csv(outdir / "private_B6OT_bridge_access_events.csv", index=False)
    comps.to_csv(outdir / "private_B6OT_control_comparison.csv", index=False)
    access_summary.to_csv(outdir / "private_B6OT_access_summary.csv", index=False)
    bridge_comp.to_csv(outdir / "private_B6OT_bridge_variant_comparison.csv", index=False)
    variant_agg.to_csv(outdir / "private_B6OT_bridge_variant_aggregate.csv", index=False)
    ablation_summary.to_csv(outdir / "private_B6OT_factor_ablation_summary.csv", index=False)
    policies.to_csv(outdir / "private_B6OT_generated_policies.csv", index=False)
    write_report(outdir / "private_B6OT_structured_phase_bridge_minimality_summary.md", access_summary, bridge_comp, variant_agg, ablation_summary, args)
    print("\nPrivate B6O-T structured phase-bridge minimality outputs")
    print(outdir)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", type=Path, default=REPO / "reports/private_b6ot_structured_phase_bridge_minimality")
